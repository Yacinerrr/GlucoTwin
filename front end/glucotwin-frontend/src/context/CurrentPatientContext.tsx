...
const CurrentPatientContext = createContext<CurrentPatientContextType | undefined>(undefined);

export const useCurrentPatient = () => {
  const context = useContext(CurrentPatientContext);
  if (!context) {
    throw new Error('useCurrentPatient must be used within CurrentPatientProvider');
  }
  return context;
};
